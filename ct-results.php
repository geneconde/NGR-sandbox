	...
	<?php
		...
		foreach($question_set as $row) { 
				if(in_array($row['qid'], $qid)) {
					...
					if		($answers[$ctr] == "A") $i = 0;
					else if	($answers[$ctr] == "B") $i = 1;
					else if ($answers[$ctr] == "C") $i = 2;
					else if ($answers[$ctr] == "D") $i = 3;
					else if ($answers[$ctr] == "None")	$i = 4;
	?>
	<tr class="trline">
		<td><img class="img-answer" /><?php echo $row['question']; ?><br/><br/>
		<?php echo _("Correct Answer"); ?>: <span class="c-answer"><?php echo $row['answer'] ?></span>. <?php echo $choices[$order]; ?><br/>
		<?php echo _("Student Answer"); ?>: <span class="s-answer"><?php echo $answers[$ctr]; ?></span>. <?php if($i != 4) echo $choices[$i]; ?>
		</td>
	</tr>
	<?php
				$ctr++;
