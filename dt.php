<?php 
	...
	$sdt_set		= $sdt->getStudentDt($userid, $mid, $mode);
...
<?php  $_SESSION['pre_dt_taken'] = 1; ?>
<h1><?php echo _("Pre-Diagnostic Test"); ?></h1>
<?php echo _("You have"); ?> <?php echo $timelimit; ?> <?php echo _("minutes to answer the questions below."); ?>
<?php } else { ?>
<h1><?php echo _("Post-Diagnostic Test"); ?></h1>
<?php  $_SESSION['post_dt_taken'] = 1; ?>
<?php echo _("You have"); ?> <?php $timelimit; ?> <?php echo _("minutes to answer the questions below."); ?>
<?php } ?>
<form action="save-dt.php?moduleid=<?php echo $mid; ?>&mode=<?php echo $mode; ?>" method="post">
	<table border="0" class="result morepad">
		<tr>
			<td colspan="2" class="bold"><?php echo _("Time Left:"); ?> <span id="note" style="color:green;"></span><br/> <div id="countdown" dir="ltr"></div><br/></td>
		</tr>
		<tr>
...
<a href="dt-results.php?module_id=<?php echo $mid; ?>&mode=<?php echo $_GET['mode']; ?>&user_id=<?php echo $userid; ?>" class="hidden button1" id="check-results"><?php echo _("Check Results"); ?></a>
</div>
...
$(document).ready(function() {
	...
	$('#submit-test').click(function(e) {
		...
		if(ctr > 0) {
			...
			document.location.href = "save-dt.php?moduleid=<?php echo $mid; ?>&mode=<?php echo $mode; ?>";
		}
	});
...
$(function(){
	var note = $('#note'),
		...
		ts = localStorage.getItem('<?php echo $userid."-".$mid."-".$mode; ?>-date');
	...
	if(expiration < ts) {
		...
		//Added by Jone for retaining the countdown timer after page refresh
		localStorage.setItem('<?php echo $userid."-".$mid."-".$mode ?>-date', ts);
		//END	
	}
