<?php 
	...
	if($user->getType() == 2) {
		...
		$sa = $uf->getStudentAnswer($_SESSION['smid'], 'doing-science-qc3-a');
		$answered = ($sa ? 1 : 0 );
	} else $answered = 1;
?>
<!DOCTYPE html>
...
#container{ width:99%; margin:10px 0 0 5px; position: relative; z-index: 6; }
#choices { 
	width: 35%; 
	list-style: none; 
	float: left;
...
$('#choices li').css({'margin':'0 0 15px 5px'});
		...
		checkAnswer();
			...
			if($('#screen1').is(':visible')) { 				
				$('#screen1').fadeOut( function(){			
					...
					$('#choices li').css({'margin':'0 0 15px 33px', 'height':'43px', 'width':'240px'});
...
function checkAnswer() {
	...
	answerVars = ['answer1', 'answer2', 'answer3', 'answer4', 'answer5', 'answer6'];
...
function save(){
	if(answered == 0){
		...
		saveAnswer('doing-science-qc3-a', answerVars[1]);	
		saveAnswer('doing-science-qc3-b', answerVars[2]);	
		saveAnswer('doing-science-qc3-c', answerVars[3]);	
		saveAnswer('doing-science-qc3-d', answerVars[4]);	
		saveAnswer('doing-science-qc3-e', answerVars[5]);	
		saveAnswer('doing-science-qc3-f', answerVars[6]);	
	}
	answered = 1;		
}
</script>
