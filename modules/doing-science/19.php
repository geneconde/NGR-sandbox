<?php 
	...
	if($user->getType() == 2) {
		...
		$sa = $uf->getStudentAnswer($_SESSION['smid'], 'doing-science-qq6-a');
		$answered = ($sa ? 1 : 0 );
	} else $answered = 1;
?>
<!DOCTYPE html>
...
#text-box {float:left; width: 50%;}
#graph {float:right; width:40%; }
#graph img {width:100%; height:200px;}
.legend { color: indigo;}
.small_font {font-size:20px;}
.smaller_font {font-size:18px;}
.mr{margin-right:80px;}
#legends {width:100%; margin-top:5px;}
#legend1 {margin-left:28px; width:10%; height:40px; background-color:#ff6633; float:left;}
#legend2 {width:10%; height:40px; background-color:#ff33ff; float:left; }
.no-mleft {margin-left:-9px !important;}
			...
			<div id="screen1" class="screens">				
				<section>					
					...
					<div id="text-box">
						...
						<div id="legends">
							<div id="legend1"></div><p class="legend allignment smaller_font fl no-mleft"><?php echo _("Dark-colored Mice");?></p>
							<div id="legend2"></div><p class="legend allignment smaller_font fl no-mleft"><?php echo _("Light-colored Mice");?></p>
							<div class="clear"></div>
						</div>										
					</div>
					<div id="population">
...
function save(){
	if(answered == 0){
		saveAnswer('doing-science-qq6-a', studentAnswer);					
		answered = 1;	
	}
}
function checkNull() {
