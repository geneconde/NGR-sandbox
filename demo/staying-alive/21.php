<?php
	...
	if($user->getType() == 2) {
		...
		$sa = $sac->getStudentAnswer($_SESSION['smid'], 'staying-alive-qq6-a');
		$answered = ($sa ? 1 : 0 );
	} else $answered = 1;
?>
<!DOCTYPE html>
...
<link rel="stylesheet" type="text/css" href="css/locale.css" />
...
@media screen and (max-width: 1250px) {
	...
	#diagram div { padding: 0px !important; }
	#diagram #pond { padding: 0 2px !important; }
	#diagram #mayfly { margin-top: 350px; }
	#diagram #trout { margin-top: 290px; }
	#diagram #bullfrog { margin-top: 290px; }
	#diagram #beaver { margin-top: 17px }
	#diagram #lilies { margin-top: 100px; }
	#diagram #duck { margin-top: 140px; }
	#diagram { height: 450px; }
	#close { width: 150px; padding: 1px; }
	#buttons { margin-left: -8px; }
}
@media screen and (max-width: 740px) {
	.ac-swirl li { margin-top: -10px; }
	.ac-swirl li label { font-size: 21px !important; }
}
</style>
</head>
<body>
	...
	question.find('input[type=radio]').on('click', function() {
		...
		if (radio.attr('id') == 'a') {
			...
			answer.find('.feedback').html('<p class="green"><img src="assets/correct.png"><?php echo _("Yes, that is correct."); ?></p>');
			ans = 'A';
		}

		if (radio.attr('id') == 'b') {
			answer.find('.answer').html('<?php echo _("Water lilies live in beaver ponds where they eat mayflies and bullfrogs."); ?>');
			answer.find('.feedback').html('<p class="red"><img src="assets/wrong.png"><?php echo _("Sorry, that is incorrect."); ?></p>');
			ans = 'B';
		}

		if (radio.attr('id') == 'c') {
			answer.find('.answer').html('<?php echo _("Wolves eat bullfrogs who build ponds where trout eat mayflies."); ?>');
			answer.find('.feedback').html('<p class="red"><img src="assets/wrong.png"><?php echo _("Nope, that is incorrect."); ?></p>');
			ans = 'C';
		}

		if (radio.attr('id') == 'd') {
			answer.find('.answer').html('<?php echo _("Bullfrogs live in beaver ponds where they eat water lilies and wood ducks."); ?>');
			answer.find('.feedback').html('<p class="red"><img src="assets/wrong.png"><?php echo _("Not really. Try again."); ?></p>');
			ans = 'D';
		}
	});
	...
	$('#click').click(function() {
		...
		var endpointOptions = { isTarget:true,container:$('#diagram'), connector:["Straight"], endpoint:["Dot", {radius:5}] };
			jsPlumb.connect({source:"wolf", target:"beaver",overlays:[["Arrow", { location:.6}], [ "Label", { label:"<p><?php echo _("Eats"); ?></p>", location:0.4} ] ], paintStyle:{lineWidth:2,strokeStyle:'#ffddbd'} }, endpointOptions );
			jsPlumb.connect({source:"wolf", target:"duck",overlays:[["Arrow", { location:.4}], [ "Label", { label:"<p><?php echo _("Eats"); ?></p>", location:0.2} ] ], paintStyle:{lineWidth:2,strokeStyle:'#ffddbd'} }, endpointOptions );
		
			jsPlumb.connect({source:"beaver", target:"pond", overlays:[["Arrow", { location:.4}], [ "Label", { label:"<p><?php echo _("Builds"); ?></p>", location:0.2 } ] ], paintStyle:{lineWidth:2,strokeStyle:'#c9a0c7'}}, endpointOptions );
			jsPlumb.connect({source:"beaver", target:"lilies",overlays:[["Arrow", { location:.6}], [ "Label", { label:"<p><?php echo _("Eats"); ?></p>", location:0.4} ] ], paintStyle:{lineWidth:2,strokeStyle:'#c9a0c7'} }, endpointOptions );
			...
			jsPlumb.connect({source:"trout", target:"pond",overlays:[["Arrow", { location:.85}], [ "Label", { label:"<p><?php echo _("Lives in"); ?></p>", location:0.7} ] ], paintStyle:{lineWidth:2,strokeStyle:'#d47d7d'} }, endpointOptions );
		
			jsPlumb.connect({source:"mayfly", target:"pond",overlays:[["Arrow", { location:.8}], [ "Label", { label:"<p><?php echo _("Lives in"); ?></p>", location:0.5} ] ], paintStyle:{lineWidth:2,strokeStyle:'#c4d080'} }, endpointOptions );
			...
			jsPlumb.connect({source:"bullfrog", target:"pond",overlays:[["Arrow", { location:.9}], [ "Label", { label:"<p><?php echo _("Lives in"); ?></p>", location:0.6} ] ], paintStyle:{lineWidth:2,strokeStyle:'#ffc27c'} }, endpointOptions );
			...
			jsPlumb.connect({source:"duck", target:"trout",overlays:[["Arrow", { location:.8}], [ "Label", { label:"<p><?php echo _("Eats"); ?></p>", location:0.7} ] ], paintStyle:{lineWidth:2,strokeStyle:'#d889ff'} }, endpointOptions );
